...
import "./IERC1400Sender.sol";
import "./IERC1400Recipient.sol";
...
contract ERC1400 is IERC1400,IERC20{
  ...
  // keccak256("ERC1400TokensSender")
  bytes32 constant private TOKENS_SENDER_INTERFACE_HASH = 0x7e6ac51ae927b5f28e4f4d24b038d87f30bf6b127235d5253403aba0f190110b;
  // keccak256("ERC1400TokensRecipient")
  bytes32 constant private TOKENS_RECIPIENT_INTERFACE_HASH = 0x2c93a7aca686281a8c69c278068a355318b4c196f673d0b4a4407212b40df37a;
...
//   mapping (bytes32 => uint256) internal _PartitionwithLock;
  ...
  */
    ...
    public
  {
    ...
    // register interfaces
    ERC1820_REGISTRY.setInterfaceImplementer(address(this), keccak256("ERC1400Token"), address(this));
  ...
  * @param _data Information attached to the issuance, by the issuer. 
  */
  ...
  * @param _data Information attached to the redemption, by the redeemer. 
  */
  ...
  * @param _operatorData Information attached to the redemption, by the operator. 
  */
  ...
  * implementing 'erc1400tokenHolder'.
  * @return Destination partition.
  */
  ...
  */
  function _callTokensToSend(address _operator,address _from,address _to,uint256 _amount,bytes memory _userData,bytes memory _operatorData) private{
    ...
    if (implementer != address(0)) {
        IERC1400Sender(implementer).tokensToSend(_operator,_from,_to,_amount,_userData,_operatorData);
    }
  }
 /**
  ...
  */
  function _callTokensReceived(address _operator,address _from,address _to,uint256 _amount,bytes memory _userData,bytes memory _operatorData,bool _requireReceptionAck)private{
    ...
    if (implementer != address(0)) {
        IERC1400Recipient(implementer).tokensReceived(_operator,_from,_to,_amount,_userData,_operatorData);
    }
  }
  ...
  */
   ...
   * Function used for ERC1400 and ERC20 backwards compatibility.
   ...
   * @param _partitions partitions to use by default when not specified.
   */
  function setDefaultPartitions(bytes32[] calldata _partitions) external onlyOwner {
    _defaultPartitions = _partitions;
  }
  ...
  */
  function balanceOf(address _tokenHolder) public view returns (uint256) {
    for(uint i=0;i<_defaultPartitions.length;i++){
        return _balanceOfByPartition[_tokenHolder][_defaultPartitions[i]];
    }
  }
  /**
  ...
  function transfer(address _recipient, uint256 _amount) external returns (bool) {
    ...
    _callTokensToSend(_from,_from,_recipient,_amount, "", "");
    for(uint i=0;i<_defaultPartitions.length;i++){
        _transferByPartition(_defaultPartitions[i], msg.sender, msg.sender,_recipient,_amount,"", "",true);
    }
    _callTokensReceived(_from,_from,_recipient,_amount, "", "", false);
    return true;
    }
  /**
  ...
  function transferFrom(address _holder, address _recipient, uint256 _amount) external returns (bool) {
    ...
    _callTokensReceived(_spender,_holder,_recipient,_amount, "", "", false);
    return true;
    }
